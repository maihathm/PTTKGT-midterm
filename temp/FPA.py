...
def pollination_local(position, best_global, flower=0, nb_flower_1=0, nb_flower_2=1, min_values=[-5, -5],
    ...
    for j in range(0, len(min_values)):
        ...
        x[j] = np.clip((position[flower, j] + r * (position[nb_flower_1, j] - position[nb_flower_2, j])), min_values[j],
                       max_values[j])
    x[-1] = target_function(x[0:len(min_values)])
    return x
...
def flower_pollination_algorithm(flowers=3, min_values=[-5, -5], max_values=[5, 5], iterations=50, gama=0.5, lamb=1.4,
                                 p=0.8, target_function=target_function):
    count = 0
    ...
    while (count <= iterations):
        ...
        for i in range(0, position.shape[0]):
            nb_flower_1 = int(np.random.randint(position.shape[0], size=1)[0])

            nb_flower_2 = int(np.random.randint(position.shape[0], size=1)[0])
...
def six_hump_camel_back(variables_values=None):
    if variables_values is None:
        variables_values = [0, 0]
...
fpa = flower_pollination_algorithm(flowers=5, min_values=[-5, -5], max_values=[5, 5], iterations=50, gama=0.1,
                                   lamb=1.5, p=0.8, target_function=six_hump_camel_back)
print(f"FPA: {fpa}")
